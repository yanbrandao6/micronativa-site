...
export function AdminShell({ children, email, role }: { children: React.ReactNode; email: string; role: AdminRole }) {
  ...
  const sidebar = <div className="flex h-full flex-col bg-navy p-5 text-white">
    <div className="flex items-center gap-3 border-b border-white/10 pb-5"><img src="/images/brand/micronativa-logo-transparent.png" alt="" className="size-11 object-contain drop-shadow-md" /><div><p className="text-sm font-extrabold tracking-wide">MICRONATIVA</p><p className="text-xs text-white/60">Painel administrativo</p></div></div>

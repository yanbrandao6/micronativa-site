...
export function SiteHeader() {
  ...
  return <header className="sticky top-0 z-50 border-b border-navy/10 bg-warm/95 backdrop-blur">
    <a href="#conteudo" className="absolute -top-20 left-4 z-[60] rounded-lg bg-navy px-4 py-3 text-white focus:top-3">Pular para o conte?do</a>
    <div className="container-site flex min-h-20 items-center justify-between gap-6">
      <Link href="/" className="flex items-center gap-3" aria-label="Micronativa ? p?gina inicial">
      ...
      <nav aria-label="Navega??o principal" className="hidden items-center gap-1 lg:flex">
        <Link className="rounded-full px-4 py-3 text-sm font-semibold hover:bg-white" href="/">In?cio</Link>
        <div className="group relative">
          <Link className="flex items-center gap-1 rounded-full px-4 py-3 text-sm font-semibold hover:bg-white" href="/solucoes">Solu??es <ChevronDown className="size-4" /></Link>
      ...
      <div className="hidden lg:block"><Link href="/orcamento" className="btn-primary">Solicitar or?amento</Link></div>
    ...
    <div id="menu-mobile" className={cn("border-t bg-white lg:hidden", open ? "block" : "hidden")}>
      <nav className="container-site grid gap-1 py-4" aria-label="Navega??o m?vel">
        {[["In?cio","/"],["Solu??es","/solucoes"],["Projetos","/projetos"],["Sobre","/sobre"],["Contato","/contato"]].map(([label, href]) => <Link key={href} onClick={() => setOpen(false)} href={href} className="rounded-xl px-4 py-3 font-semibold hover:bg-warm">{label}</Link>)}
        <p className="px-4 pt-3 text-xs font-bold uppercase tracking-wider text-muted">Servi?os</p>
        ...
        <Link onClick={() => setOpen(false)} href="/orcamento" className="btn-primary mt-3">Solicitar or?amento</Link>
      </nav>
    </div>
  </header>;
}

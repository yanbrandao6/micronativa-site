...
export function ProjectCard({ project }: { project: ProjectCaseStudy }) {
  return <article className="card card-hover group overflow-hidden">
    <div className="relative aspect-[4/3] overflow-hidden bg-navy-pale"><Image src={project.coverImage.src} alt={project.coverImage.alt} fill className="object-cover transition duration-500 group-hover:scale-[1.03]" /></div>
    <div className="p-6"><div className="flex flex-wrap gap-2"><span className="rounded-full bg-forest-pale px-3 py-1 text-xs font-bold text-forest">{project.categoryLabel}</span>{project.isPlaceholder && <span className="rounded-full bg-solar/25 px-3 py-1 text-xs font-bold text-ink">Demonstra??o</span>}</div>
